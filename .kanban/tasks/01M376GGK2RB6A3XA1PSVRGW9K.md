...
- The run id: `AgentRun.id` is a `ULID` that is also the session id and the recording folder name. The run makes the ULID at start, before it renders the body, and gives it to the session. Thus a run that fails before it makes a session still has an id. The tool-call `completionToken` is only a lookup key in the runner index, not the id. (Plan.md §8.1 wins over §9.2; update §9.2 of `plan.md` in this task to say this.) If `makeSession` does not take a session id, add a Router task and use the id that the session gives, and record the gap here.
...
- `Sources/FoundationModelsAgents/Run/AgentRun.swift`: `public final class AgentRun: Sendable` with `id: ULID`, `agent`, `caller: ULID?`, `depth`, `state`, `recordingDirectory`, `result() async throws -> String`, `cancel()`.
  ...
  6. Drive one turn with `session.streamEvents(to: prompt)`. Nothing goes to the caller during the turn.
...
- [ ] A body render failure gives `.failed(.bodyRenderFailed)`, makes no session, and the run still has an id.
...
- [ ] A run started inside a tool call of a Router session records `agentSpawn` in its `session.json`, and `parentToolCallId` joins to that tool call in the Router transcript (plan.md §16); a host-driven run records none.
